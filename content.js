...
panel.innerHTML =
  ...
  '<div class="af-body">' +
    ...
    '<div class="af-group"><label>Filter Tanggal (opsional):</label><div style="display:flex;gap:4px;align-items:center"><span style="font-size:10px">Dari</span><input type="date" id="af-date-from" style="flex:1" /><span style="font-size:10px">s/d</span><input type="date" id="af-date-to" style="flex:1" /></div></div>' +
    '<div class="af-group"><label>Mulai dari entry ke-:</label><input type="number" id="af-start-entry" value="1" min="1" /><span style="font-size:10px;color:#999">(default: 1, dihitung setelah filter tanggal)</span></div>' +
...
document.getElementById('af-btn-run').addEventListener('click', function() {
  ...
  var dateFrom = document.getElementById('af-date-from').value || '';
  var dateTo = document.getElementById('af-date-to').value || '';
  getData(function(data) {
    if (!data) { addLog('Tidak ada data!', 'err'); return; }
    var bd = data.breakdowns[bdIdx]; if (!bd) { addLog('Breakdown tidak valid!', 'err'); return; }

    var filteredEntries = bd.entries;
    if (dateFrom || dateTo) {
      var fromNum = dateFrom ? parseInt(dateFrom.replace(/-/g, ''), 10) : 0;
      var toNum = dateTo ? parseInt(dateTo.replace(/-/g, ''), 10) : 99999999;
      filteredEntries = bd.entries.filter(function(e) {
        var p = e.tanggal ? e.tanggal.split('-') : [];
        if (p.length !== 3 || p[0].length !== 2) return true;
        var num = parseInt(p[2] + p[1] + p[0], 10);
        return num >= fromNum && num <= toNum;
      });
      addLog('Filter tanggal: ' + (dateFrom || '...') + ' s/d ' + (dateTo || '...') + ' → ' + filteredEntries.length + '/' + bd.entries.length + ' entry', 'info');
      if (filteredEntries.length === 0) {
        addLog('Tidak ada entry dalam rentang tanggal!', 'err');
        return;
      }
      bd = JSON.parse(JSON.stringify(bd));
      bd.entries = filteredEntries;
      bd.jumlah_entries = filteredEntries.length;
      data = JSON.parse(JSON.stringify(data));
      data.breakdowns[bdIdx] = bd;
      chrome.storage.local.set({ emaster_activities: data });
    }
